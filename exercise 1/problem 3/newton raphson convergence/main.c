...
//the result of newton raphson is investigated for a variety of values.         //
//////////////////////////////////////////////////////////////////////////////////
...
int main()
{
    ...
    { // Allocation succeeded. program continues:
        ...
        for (i=lwr; i<uppr; i++)
        {//output a number of newton raphson results to a file
            FILE* convergence;
            convergence= fopen("N_R_investigation.txt", "w");
            if (convergence!=NULL)
            {
                //check that the derivative at start_value is not zero, otherwise the method will fail
                if (check_derivative( differentiated, i, eqn_order)==1)
                {
                ...
                {//calculate the root.
                    printf("\nIterating....\n");
                    Xn [0] =0;
                    Xn [1]= calculate_Xn(i, eqn, differentiated, eqn_order);//initial approximation
                    double Xtemp=i;
                    int p;
                    //iterate, stopping either when two subsequent iterations are essentially the same, or after 100 iterations
                    for (p=0; p<=100, sqrt((Xn[1]-Xtemp)*(Xn[1]-Xtemp))>0.000001;p++)
                    {
            ...
            fprintf(convergence, "%d\t", i);//print the start value

            //print the root found:
            {
                if (Xn [0] == 100)
                    fprintf(convergence, "%lf\t", Xn[0]);//error value is 100
                else
                {//succesful iteration
                    fprintf(convergence,"%lf\t", Xn [1]);
